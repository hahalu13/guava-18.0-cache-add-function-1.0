...
public final class CacheBuilder<K, V> {
	...
	long expireAfterCreateNanos = UNSET_INT;
	
	public CacheBuilder<K, V> expireAfterCreate(long duration, TimeUnit unit) {
		checkState(expireAfterCreateNanos == UNSET_INT, "expireAfterCreate was already set to %s ns",
				expireAfterCreateNanos);
		checkArgument(duration >= 0, "duration cannot be negative: %s %s", duration, unit);
		this.expireAfterCreateNanos = unit.toNanos(duration);
		return this;
	}
	
	long getExpireAfterCreateNanos() {
		return (expireAfterCreateNanos == UNSET_INT) ? DEFAULT_EXPIRATION_NANOS : expireAfterCreateNanos;
	}
