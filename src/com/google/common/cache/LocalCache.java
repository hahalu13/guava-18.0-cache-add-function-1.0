...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	...
	LocalCache(
		...
		expireAfterCreateNanos=builder.getExpireAfterCreateNanos();
		...
		removalNotificationQueue = (removalListener == NullListener.INSTANCE)
				...
				entryFactory = EntryFactory.getFactory(keyStrength, usesAccessEntries(), usesWriteEntries(),usesCreateEntries());
	...
	boolean recordsTime() {
		return recordsWrite() || recordsAccess() || recordsCreate();
	}
	...
	/*************modify by wanggh on 2014 09 11 *********/
	final long expireAfterCreateNanos;

	boolean expiresAfterCreate() {
		return expireAfterCreateNanos > 0;
	}

	boolean recordsCreate() {
		return expiresAfterCreate();
	}

	boolean usesCreateQueue() {
		return expiresAfterCreate();
	}

	boolean usesCreateEntries() {
		return usesCreateQueue() || recordsCreate();
	}
	/**********************/
	...
	enum EntryFactory {
		STRONG {
			...
			<K, V> ReferenceEntry<K, V> newEntry(
					Segment<K, V> segment, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
				return new StrongEntry<K, V>(key, hash, next,createTime,timeout);
			}
		},
		STRONG_ACCESS {
			@Override
			<K, V> ReferenceEntry<K, V> newEntry(
					Segment<K, V> segment, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
				return new StrongAccessEntry<K, V>(key, hash, next,createTime,timeout);
			}
		...
		STRONG_WRITE {
			...
			<K, V> ReferenceEntry<K, V> newEntry(
					Segment<K, V> segment, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
				return new StrongWriteEntry<K, V>(key, hash, next,createTime,timeout);
			}
		...
		STRONG_ACCESS_WRITE {
			...
			<K, V> ReferenceEntry<K, V> newEntry(
					Segment<K, V> segment, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
				return new StrongAccessWriteEntry<K, V>(key, hash, next,createTime,timeout);
			}
		...
		WEAK {
			...
			<K, V> ReferenceEntry<K, V> newEntry(
					Segment<K, V> segment, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
				return new WeakEntry<K, V>(segment.keyReferenceQueue, key, hash, next,createTime,timeout);
			}
		},
		WEAK_ACCESS {
			@Override
			<K, V> ReferenceEntry<K, V> newEntry(
					Segment<K, V> segment, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
				return new WeakAccessEntry<K, V>(segment.keyReferenceQueue, key, hash, next,createTime,timeout);
			}
		...
		WEAK_WRITE {
			...
			<K, V> ReferenceEntry<K, V> newEntry(
					Segment<K, V> segment, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
				return new WeakWriteEntry<K, V>(segment.keyReferenceQueue, key, hash, next,createTime,timeout);
			}
		...
		WEAK_ACCESS_WRITE {
			...
			<K, V> ReferenceEntry<K, V> newEntry(
					Segment<K, V> segment, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
				return new WeakAccessWriteEntry<K, V>(segment.keyReferenceQueue, key, hash, next,createTime,timeout);
			}
		...
		static final int CREATE_MASK =8;
		...
		static EntryFactory getFactory(Strength keyStrength, boolean usesAccessQueue,
				boolean usesWriteQueue,boolean usesCreateQueue) {
			int flags = ((keyStrength == Strength.WEAK) ? WEAK_MASK : 0)
					| (usesAccessQueue ? ACCESS_MASK : 0)
					| (usesWriteQueue ? WRITE_MASK : 0)
					| (usesCreateQueue ? WRITE_MASK : 0)
					;
		...
		<K, V> ReferenceEntry<K, V> copyEntry(
				Segment<K, V> segment, ReferenceEntry<K, V> original, ReferenceEntry<K, V> newNext) {
			return newEntry(segment, original.getKey(), original.getHash(), newNext,original.getCreateTime(),original.getTimeout());
		}
	...
	interface ReferenceEntry<K, V> {
		...
		ReferenceEntry<K, V> getNextInCreateQueue();

		void setNextInCreateQueue(ReferenceEntry<K, V> next);

		ReferenceEntry<K, V> getPreviousInCreateQueue();

		void setPreviousInCreateQueue(ReferenceEntry<K, V> previous);
		/****************************/
	}
	...
	private enum NullEntry implements ReferenceEntry<Object, Object> {
		...
		@Override
		public ReferenceEntry<Object, Object> getNextInCreateQueue() {
			return this;
		}

		@Override
		public void setNextInCreateQueue(ReferenceEntry<Object, Object> next) {}

		@Override
		public ReferenceEntry<Object, Object> getPreviousInCreateQueue() {
			return this;
		}

		@Override
		public void setPreviousInCreateQueue(ReferenceEntry<Object, Object> previous) {}
		/*******************************/
	}
	...
	static abstract class AbstractReferenceEntry<K, V> implements ReferenceEntry<K, V> {
		...
		@Override
		public ReferenceEntry<K, V> getNextInCreateQueue() {
			throw new UnsupportedOperationException();
		}

		@Override
		public void setNextInCreateQueue(ReferenceEntry<K, V> next) {
			throw new UnsupportedOperationException();
		}

		@Override
		public ReferenceEntry<K, V> getPreviousInCreateQueue() {
			throw new UnsupportedOperationException();
		}

		@Override
		public void setPreviousInCreateQueue(ReferenceEntry<K, V> previous) {
			throw new UnsupportedOperationException();
		}
	...
	static class StrongEntry<K, V> extends AbstractReferenceEntry<K, V> {
		...
		StrongEntry(K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
			this.key = key;
			this.hash = hash;
			this.next = next;
		...
		/****************modify by wanggh***************/
		ReferenceEntry<K, V> nextCreate = nullEntry();
		...
		@Override
		public void setNextInCreateQueue(ReferenceEntry<K, V> next) {
			this.nextCreate = next;
		}

		ReferenceEntry<K, V> previousCreate = nullEntry();

		@Override
		public ReferenceEntry<K, V> getPreviousInCreateQueue() {
			return previousCreate;
		}

		@Override
		public void setPreviousInCreateQueue(ReferenceEntry<K, V> previous) {
			this.previousCreate = previous;
		}
	}

	static final class StrongAccessEntry<K, V> extends StrongEntry<K, V> {
		StrongAccessEntry(K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
			super(key, hash, next,createTime,timeout);
		}
	...
	static final class StrongWriteEntry<K, V> extends StrongEntry<K, V> {
		StrongWriteEntry(K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
			super(key, hash, next,createTime,timeout);
		}
	...
	static final class StrongAccessWriteEntry<K, V> extends StrongEntry<K, V> {
		StrongAccessWriteEntry(K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
			super(key, hash, next,createTime,timeout);
		}
	...
	static class WeakEntry<K, V> extends WeakReference<K> implements ReferenceEntry<K, V> {
		WeakEntry(ReferenceQueue<K> queue, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
			super(key, queue);
			this.hash = hash;
			this.next = next;
			this.createTime=createTime;
	...
	static final class WeakAccessEntry<K, V> extends WeakEntry<K, V> {
		WeakAccessEntry(
				ReferenceQueue<K> queue, K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
			super(queue, key, hash, next,createTime,timeout);
		}
	 ...
	 */
	@VisibleForTesting
	ReferenceEntry<K, V> newEntry(K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
		Segment<K, V> segment = segmentFor(hash);
		segment.lock();
		try {
			return segment.newEntry(key, hash, next,createTime,timeout);
		} finally {
			segment.unlock();
		}
	}
	...
	boolean isExpired(ReferenceEntry<K, V> entry, long now) {
		...
		if (expiresAfterAccess()
				&& (now - entry.getAccessTime() >= (entry.getTimeout() ==0 ? expireAfterAccessNanos:entry.getTimeout()) )) {
			return true;
		}
		if (expiresAfterWrite()
				&& (now - entry.getWriteTime() >= (entry.getTimeout() ==0 ? expireAfterWriteNanos:entry.getTimeout()))) {
			return true;
		}

		/**********modify by wanggh**********/
		if (expiresAfterCreate()
				&& (now - entry.getCreateTime() >= (entry.getTimeout() ==0 ? expireAfterCreateNanos:entry.getTimeout()))) {
			return true;
		}
		return false;
	}
	...
	/*************modify by wanggh***************/
	static <K, V> void connectCreateOrder(ReferenceEntry<K, V> previous, ReferenceEntry<K, V> next) {
		previous.setNextInCreateQueue(next);
		next.setPreviousInCreateQueue(previous);
	}

	// Guarded By Segment.this
	static <K, V> void nullifyCreateOrder(ReferenceEntry<K, V> nulled) {
		ReferenceEntry<K, V> nullEntry = nullEntry();
		nulled.setNextInCreateQueue(nullEntry);
		nulled.setPreviousInCreateQueue(nullEntry);
	}
	/*******************************/
	...
	static class Segment<K, V> extends ReentrantLock {
		...
		/**************modify by wanggh*************/
		final Queue<ReferenceEntry<K, V>> createQueue;
							...
							recencyQueue = map.usesAccessQueue()
									...
									writeQueue = map.usesWriteQueue()
											...
											accessQueue = map.usesAccessQueue()
													...
													createQueue= map.usesCreateQueue()
															? new CreateQueue<K, V>()
																	: LocalCache.<ReferenceEntry<K, V>>discardingQueue();
		}
		...
		@GuardedBy("this")
		ReferenceEntry<K, V> newEntry(K key, int hash, @Nullable ReferenceEntry<K, V> next,long createTime,long timeout) {
			return map.entryFactory.newEntry(this, checkNotNull(key), hash, next,createTime,timeout);
		}
		...
		V lockedGetOrLoad(K key, int hash, CacheLoader<? super K, V> loader)
			...
			try {
				...
				if (createNewEntry) {
					...
					if (e == null) {
						e = newEntry(key, hash, first,now,0);//TODO:MODIFY 
		...
		void expireEntries(long now) {
			...
			/***********modify by wanggh************/
			while ((e = createQueue.peek()) != null && map.isExpired(e, now)) {
				if (!removeEntry(e, e.getHash(), RemovalCause.EXPIRED)) {
					throw new AssertionError();
				}
			}
		...
		V put(K key, int hash, V value, boolean onlyIfAbsent) {
			...
			try {
				...
				// Create a new entry.
				++modCount;
				ReferenceEntry<K, V> newEntry = newEntry(key, hash, first,now,0);
		...
		@Nullable
		V put(K key, int hash, V value, boolean onlyIfAbsent,long timeout) {
			lock();
			try {
				long now = map.ticker.read();
				preWriteCleanup(now);
				...
				// Create a new entry.
				++modCount;
				ReferenceEntry<K, V> newEntry = newEntry(key, hash, first,now,timeout);
				setValue(newEntry, key, value, now);
				table.set(index, newEntry);
				newCount = this.count + 1;
				this.count = newCount; // write-volatile
		...
		boolean storeLoadedValue(K key, int hash, LoadingValueReference<K, V> oldValueReference,
			...
			try {
				...
				++modCount;
				ReferenceEntry<K, V> newEntry = newEntry(key, hash, first,now,0);//TODO:MODIFY
	...
	static final class CreateQueue<K, V> extends AbstractQueue<ReferenceEntry<K, V>> {
		final ReferenceEntry<K, V> head = new AbstractReferenceEntry<K, V>() {
			...
			ReferenceEntry<K, V> nextCreate = this;
			...
			ReferenceEntry<K, V> previousCreate = this;
		...
		public boolean offer(ReferenceEntry<K, V> entry) {
			// unlink
			connectCreateOrder(entry.getPreviousInCreateQueue(), entry.getNextInCreateQueue());

			// add to tail
			connectCreateOrder(head.getPreviousInCreateQueue(), entry);
			connectCreateOrder(entry, head);
		...
		@Override
		public ReferenceEntry<K, V> peek() {
			ReferenceEntry<K, V> next = head.getNextInCreateQueue();
			return (next == head) ? null : next;
		}
		...
		public boolean remove(Object o) {
			ReferenceEntry<K, V> e = (ReferenceEntry) o;
			ReferenceEntry<K, V> previous = e.getPreviousInCreateQueue();
			ReferenceEntry<K, V> next = e.getNextInCreateQueue();
			connectCreateOrder(previous, next);
			nullifyCreateOrder(e);
		...
		public boolean contains(Object o) {
			ReferenceEntry<K, V> e = (ReferenceEntry) o;
			return e.getNextInCreateQueue() != NullEntry.INSTANCE;
		}

		@Override
		public boolean isEmpty() {
			return head.getNextInCreateQueue() == head;
		}

		@Override
		public int size() {
			int size = 0;
			for (ReferenceEntry<K, V> e = head.getNextInCreateQueue(); e != head;
					e = e.getNextInCreateQueue()) {
				size++;
			}
			return size;
		}
		...
		@Override
		public void clear() {
			ReferenceEntry<K, V> e = head.getNextInCreateQueue();
			while (e != head) {
				ReferenceEntry<K, V> next = e.getNextInCreateQueue();
				nullifyCreateOrder(e);
				e = next;
			...
			head.setNextInCreateQueue(head);
			head.setPreviousInCreateQueue(head);
		}
		...
		public Iterator<ReferenceEntry<K, V>> iterator() {
			return new AbstractSequentialIterator<ReferenceEntry<K, V>>(peek()) {
				@Override
				protected ReferenceEntry<K, V> computeNext(ReferenceEntry<K, V> previous) {
					ReferenceEntry<K, V> next = previous.getNextInCreateQueue();
					return (next == head) ? null : next;
				}
			};
		}
	...
	public V put(K key, V value,long timeout) {
		...
		checkNotNull(value);
		int hash = hash(key);
		return segmentFor(hash).put(key, hash, value, false,timeout);
	}
	
	public V put(K key, V value,long timeout,TimeUnit unit) {
		return put(key,value,unit.toNanos(timeout));
	}
	...
	static class LocalManualCache<K, V> implements Cache<K, V>, Serializable {
		...
		public void put(K key, V value,long timeout,TimeUnit unit) {
			localCache.put(key, value,timeout,unit);
		}
	}
